...
const WatchlistButton = ({ symbol }) => {
    ...
    return (
        <div>
            {watchlistButton ? 
                <div style={{ top: "10px" }} className="group absolute right-4">
                    ...
                    <span style={{width: "150px"}} class="absolute top-8 -left-7 text-center scale-0 rounded bg-gray-600 p-2 text-xs text-white group-hover:scale-100">Remove from watchlist</span>
                </div>
             : <button onClick={() => { addToWatchlist(symbol) }} className="absolute top-4 right-4 text-blue-500 hover:underline text-xs">Add to watchlist +</button>}
        </div>
    );
};
